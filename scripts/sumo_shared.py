...
#env variable names
ENV_SUMO_ACCESS_ID = 'SUMO_ACCESS_ID'
ENV_SUMO_ACCESS_KEY = 'SUMO_ACCESS_KEY'
ENV_SUMO_API_ENDPOINT = 'SUMO_API_ENDPOINT'
...
# todo - combine this class with the next one. this is just hack to remove SDK dependency
    def __init__(self, callerthread=None, accessId=None, accessKey=None, endpoint=None, cookieFile='cookies.txt', proxy=None, apirev=1):
        ...
        if accessId==None:
            self.session.auth = (os.environ[ENV_SUMO_ACCESS_ID], os.environ[ENV_SUMO_ACCESS_KEY])
    ...
    def get(self, method, params=None, headers=None):
        ...
        logging.debug('get detailed status = ' + str(r.status_code) + ' body = ' + r.text)
        retries = 0
        MAXRETRYATTEMPTS = 10
    ...
    def post(self, method, params, headers=None):
        ...
        logging.debug('post detailed status = ' + str(r.status_code) + ' body = ' + r.text)
        retries = 0
        MAXRETRYATTEMPTS = 10
        while 400 <= r.status_code < 600 and retries <= MAXRETRYATTEMPTS :
            retries+=1
    ...
    def collector_byName(self, collector_name):
        r = self.get('/collectors/name/' + collector_name.replace(' ', '%20')) # tbd python2 has 2 dif libs to properly do this
    ...
    def scheduledViews(self, params=None, headers=None):
        r = self.get('/scheduledViews', params, headers)
        return json.loads(r.text)

    def create_scheduledView(self, viewobj):
        return self.post('/scheduledViews', viewobj)

    def update_scheduledView(self, viewid, viewobj):
        return self.put('/scheduledViews', viewid, viewobj)

    def disable_scheduledView(self, viewid):
        return self.delete('/scheduledViews/' + viewid + '/disable')
    ...
    def getMonitorsRoot(self):
        return self.get('/monitors/root')

    def exportMonitors(self, id):
        return self.get('/monitors/' + id + '/export')
